...
void COneGraph::ReadEdgesFilePhysical(string sEdgesFileName)
{
	...
	{
		if (sOneLineContent.compare("") == 0){continue;} // if it is an empty line, continue;
		std::istringstream(sOneLineContent) >> nNode1Index >> nNode2Index;
		COneEdge aOneEdge;
...
void COneGraph::ReadEdgesFileConceptual(string sEdgesFileName)
{
	...
	while (std::getline(aOneFileInputStrm, sOneLineContent))
	{
		...
		if (nNode1Index < nNode2Index)
		{	// Since the edges are duplicate in EIN Matrix Transition Matrix file.
			COneEdge aOneEdge;
			aOneEdge.SetIndexOfNode1(nNode1Index);
			aOneEdge.SetIndexOfNode2(nNode2Index);
...
void COneGraph::CalculateNodeDegreeInConceptualNetwork(){
	...
	for (int nIdx = 0; nIdx < nNumOfNodes; nIdx++)
	{
		...
		aOneNodeRefer.SetNodeDegree(dSummationOfNodeWeight); // Yubao20151130
	}
};
...
void COneGraph::OutputTheDensestSubgraph()
{
	...
	string sOutputFileName2 = m_sOutputFilesPath + "02DenseSubgraphEdgesPhysical.txt";
	string sOutputFileName3 = m_sOutputFilesPath + "03DenseSubgraphEdgesConceptual.txt";
	...
	aOutputStream.close();
	// Output the edges in the physical network
	aOutputStream.open(sOutputFileName2, ofstream::out);
	int nNumOfEdges = m_aDensestSubnetwork.GetNumOfPPIEdges();
	for (int nIdx = 0; nIdx < nNumOfEdges; nIdx++)
	{
		COneEdge& aOneEdgeRefer = m_aDensestSubnetwork.GetOnePPIEdgeRefer(nIdx);
		int nIndexOfNode1 = aOneEdgeRefer.GetIndexOfNode1();
		int nIndexOfNode2 = aOneEdgeRefer.GetIndexOfNode2();
		double dEdgeWeight = aOneEdgeRefer.GetEdgeWeight();
	...
	aOutputStream.close();
	// Output the edges in the conceptual network
	aOutputStream.open(sOutputFileName3, ofstream::out);
	nNumOfEdges = m_aDensestSubnetwork.GetNumOfGGIEdges();
	for (int nIdx = 0; nIdx < nNumOfEdges; nIdx++)
	{
...
void COneGraph::OutputSummaryOfDCS(){
	string sOutputFileName1 = m_sOutputFilesPath + "04SummaryOfDCS.txt";
	ofstream aOutputStream;
	// Output PPI subnetwork
	aOutputStream.open(sOutputFileName1, ofstream::out);
	aOutputStream << "Number of Nodes : " << m_aDensestSubnetwork.GetNumOfNodes() << endl;
	aOutputStream << "Number of Edges in the Physical Network : " << m_aDensestSubnetwork.GetNumOfPPIEdges() << endl;
	aOutputStream << "Number of Edges in the Conceptual Network : " << m_aDensestSubnetwork.GetNumOfGGIEdges() << endl;
	aOutputStream << "Density of the Subgraph in the Conceptual Network : " << m_aSubNetworkForGreedyDeleteNode.GetDensityOfDensestSubgraph() << endl;
